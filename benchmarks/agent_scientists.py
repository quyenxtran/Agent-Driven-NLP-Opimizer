...
def scientist_a_pick(
    ...
    try:
        ...
        prompt = compact_prompt_block(prompt, max_chars=25000, max_lines=700)
    except Exception as exc:
        prompt_warning = f"Prompt build warning: {type(exc).__name__}: {exc}"
        prompt = (
...
def scientist_b_review(
    ...
    try:
        prompt = textwrap.dedent(
            ...
            Known failure modes (check if proposal risks repeating these):
            {failure_compact}
        ...
        prompt = compact_prompt_block(prompt, max_chars=22000, max_lines=600)
    except Exception as exc:
        prompt_warning = f"Prompt build warning: {type(exc).__name__}: {exc}"
        prompt = (
